module Main exposing (BioStatus(..), Model, Msg(..), RequiredStatus(..), SummaryStatus(..), bioError, buttonStyleReady, checkRequired, empty, errorStyle, init, inputStyle, isReady, main, makeLabelName, requiredError, showError, summaryError, update, validate, validateBio, validateSummary, view, viewForm, viewInput, viewTextarea, wordCount)
...
viewForm model =
    Form.form []
        ...
        , br [] []
        , button [ style "color" "green", onClick Submit, disabled (not model.ready) ]
            [ text
                ((\mdl ->
                    if mdl.ready then
                        "yes"
...
isReady model =
    model.nameValidation
        ...
        == RequiredValid
        && model.supervisorsValidation
        == RequiredValid
        && model.titleValidation
        == RequiredValid
        && model.summaryValidation
        == ValidSummary
        && model.bioValidation
        == ValidBio
